...
Usually when I look at something new, I want to see what kind of code I will have to write, so there's a full example in example/ directory of this project [github.com/nedscode/transit/example](https://github.com/nedscode/transit/example).

The first time the subscribe is called, the server will create a new inbox called "transactions.usvc/foo.bar.*" which will receive any message with a subject matching the prefix `foo.bar.`, according to the Entry rules.
...
The example publishes an entry to the server then handles the subscription directly after the message becomes available.
